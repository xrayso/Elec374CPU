...
module tb_div;
  ...
  initial begin
    ...
    // R3 / R1 => quotient 11, remainder 1
    load_reg(3, 32'd100);
    load_reg(1, 32'd9);

    fetch_instr(32'h6188_0000); // div R3, R1
    if (IR_q_dbg !== 32'h6188_0000) begin
      $display("FAIL DIV FETCH: IR=%h expected=%h", IR_q_dbg, 32'h6188_0000);
      $fatal;
    end
    ...
    if (LO_q_dbg !== 32'd11 || HI_q_dbg !== 32'd1) begin
      $display("FAIL DIV: HI=%0d LO=%0d expected HI=1 LO=11", HI_q_dbg, LO_q_dbg);
      $fatal;
    end

    $display("PASS DIV: HI=%0d LO=%0d", HI_q_dbg, LO_q_dbg);
    $finish;
  end
